...
## BRT vs BTR-stump and MaxEnt-LQP vs MaxEnt-LQ

We also tried a couple of other models to check whether the limited flexibility can affect the performance. We implemented a BRT model with a limited tree-complexity of `1` also known as **stump**. The main difference between the BRT-stump and the default BRT (with tree-complexity 1 or 5) is that the default BRT can fit a higher level of interaction between the covariates. The implemented BRT model here (Elith *et al.* 2008) utilized internal cross-validation to fund the right complexity for the mode. Thus, by limiting tree-complexity to 1, the model adds more trees to find a similar balance in the fitted model as the BRT with tree-complexity 5.    
...
cv_result <- read.csv("output/nceas_blockcv_total.csv")
brtmxnt <- read.csv("output/nceas_blockcv_brt_maxent.csv")
...
messall <- list.files("output/nceas_blockcv_mess", full.names = TRUE) %>% 
  map(read.csv) %>% 
...
messall %>% 
  group_by(species, region, cv) %>% 
  summarise(extrapolate = sum(mess < 0)) %>% # / n()
...
messall %>% 
  group_by(species, region, cv) %>% 
  summarise(extrapolate = sum(mess < 0)) %>% 
...
```{r}
knitr::kable(read.csv("Environment/AWT_environment.csv"), caption = "Environmental vartiables for AWT region.")

```

```{r}
knitr::kable(read.csv("Environment/CAN_environment.csv"), caption = "Environmental vartiables for CAN region.")

```

```{r}
knitr::kable(read.csv("Environment/NSW_environment.csv"), caption = "Environmental vartiables for NSW region.")

```

```{r}
knitr::kable(read.csv("Environment/NZ_environment.csv"), caption = "Environmental vartiables for NZ region.")

```

```{r}
knitr::kable(read.csv("Environment/SA_environment.csv"), caption = "Environmental vartiables for SA region.")

```

```{r}
knitr::kable(read.csv("Environment/SWI_environment.csv"), caption = "Environmental vartiables for SWI region.")
